...
DELETE FROM public.products WHERE vendor_id IN (SELECT id FROM public.vendors WHERE name IN ('Lapacho Pizzas', 'Sakura Sushi', 'Burger Lapacho', 'Empanadas del Norte', 'Pasta Rosa', 'Café Lapacho'));
DELETE FROM public.vendor_hours WHERE vendor_id IN (SELECT id FROM public.vendors WHERE name IN ('Lapacho Pizzas', 'Sakura Sushi', 'Burger Lapacho', 'Empanadas del Norte', 'Pasta Rosa', 'Café Lapacho'));
...
-- Insert vendor hours (Mon=1, Sun=0)
