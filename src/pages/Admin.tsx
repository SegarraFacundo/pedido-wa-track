...
export default function Admin() {
  ...
  const menuItems = [
    { value: "vendors", label: t('admin.vendors'), icon: Store },
    { value: "commissions", label: t('admin.commissions'), icon: DollarSign },
    { value: "metrics", label: t('admin.metrics'), icon: BarChart3 },
    { value: "analytics", label: t('admin.analytics'), icon: TrendingUp },
    { value: "invoices", label: t('admin.invoices'), icon: FileText },
    { value: "reports", label: t('admin.reports'), icon: BarChart3 },
    { value: "reviews", label: t('admin.reviews'), icon: Star },
    { value: "support", label: t('admin.supportTickets'), icon: Headphones },
    { value: "soporte-users", label: t('admin.supportUsers'), icon: Users },
    { value: "evolution", label: t('admin.aiAgent'), icon: Bot },
    { value: "emergency", label: t('admin.emergency'), icon: AlertTriangle, danger: true },
    { value: "tools", label: t('admin.tools'), icon: Wrench },
  ];
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-muted-foreground">{t('admin.verifyingAccess')}</p>
        </div>
      </div>
    );
  }
      ...
      <header className="border-b bg-card sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
              ...
              <SheetContent side="left" className="w-72">
                <SheetHeader><SheetTitle className="flex items-center gap-2"><img src={lapachoIcon} alt="Lapacho" className="h-6" />{t('admin.panelTitle')}</SheetTitle></SheetHeader>
                <nav className="mt-6 flex flex-col gap-1">
                  {menuItems.map((item) => (
                    <Button key={item.value} variant={activeTab === item.value ? "secondary" : "ghost"}
                      className={cn("justify-start w-full", item.danger && activeTab === item.value && "bg-orange-500 text-white hover:bg-orange-600", item.danger && activeTab !== item.value && "text-orange-600 hover:text-orange-700 hover:bg-orange-50")}
          ...
          <Button onClick={handleSignOut} variant="outline" size="sm">
            <LogOut className="mr-2 h-4 w-4" /><span className="hidden sm:inline">{t('common.signOut')}</span>
          </Button>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-8">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="hidden lg:grid w-full grid-cols-12 mb-8">
            {menuItems.map((item) => (
              <TabsTrigger key={item.value} value={item.value} className={cn(item.danger && "data-[state=active]:bg-orange-500 data-[state=active]:text-white")}>
                <item.icon className="mr-2 h-4 w-4" /><span className="hidden xl:inline">{item.label}</span>
              </TabsTrigger>
            ))}
          </TabsList>
          {currentMenuItem && (
            <div className="lg:hidden mb-4 flex items-center gap-2 px-1">
              <currentMenuItem.icon className={cn("h-5 w-5", currentMenuItem.danger ? "text-orange-500" : "text-primary")} />
              <h2 className="text-lg font-semibold">{currentMenuItem.label}</h2>
            </div>
          )}
          <TabsContent value="vendors"><VendorManagement /></TabsContent>
          <TabsContent value="commissions"><CommissionManagement /></TabsContent>
          <TabsContent value="metrics"><AdminPaymentMetrics /></TabsContent>
          <TabsContent value="analytics"><VendorChangeAnalytics /></TabsContent>
          <TabsContent value="invoices"><CommissionInvoiceGenerator /></TabsContent>
          <TabsContent value="reports"><CommissionReports /></TabsContent>
          <TabsContent value="reviews"><PlatformReviewsPanel /></TabsContent>
          <TabsContent value="support"><SupportPanel /></TabsContent>
          <TabsContent value="soporte-users"><SoporteUserManagement /></TabsContent>
          <TabsContent value="evolution"><EvolutionConfig /></TabsContent>
          <TabsContent value="emergency"><EmergencyControl /></TabsContent>
          <TabsContent value="tools">
            <div className="space-y-6">
              <h2 className="text-2xl font-bold">{t('admin.toolsTitle')}</h2>
